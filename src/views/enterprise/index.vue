<template>
  <div>
    ...
    <el-card class="box-card">
      <!-- 头部 -->
      <el-form :inline="true" :model="forment" class="demo-form-inline" ref="enterprise">
        <el-form-item label="企业编号" prop="eid">
          <el-input v-model="forment.eid"></el-input>
        </el-form-item>
        <el-form-item label="企业名称" prop="name">
          <el-input v-model="forment.name"></el-input>
        </el-form-item>
        <el-form-item label="创建者" prop="username">
          <el-input v-model="forment.username"></el-input>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select v-model="forment.status" placeholder="请选择状态">
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="addent">
    ...
    <el-card class="el-card-list">
      <el-table :data="enterpriselist" style="width: 100%">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="compileent(scope.row)">编辑</el-button>
            <el-button
              type="text"
              size="small"
              @click="changestate(scope.row)"
            >{{ scope.row.status? '禁用' : '启用' }}</el-button>
            <el-button type="text" size="small" @click="removeent(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
<script>
import { apigetent, apicreatestatus, apiremoveent } from "../../api/enterprise";
  ...
  methods: {
    ...
    getenterprise () {
      apigetent({
        ...
        page: this.pagenum,
        limit: this.pagesize,
        eid: this.forment.eid,
        username: this.forment.username,
        status: this.forment.status
      }).then(res => {
